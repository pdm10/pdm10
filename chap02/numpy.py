...
np.concatenate((x, y), axis=1) #열
np.concatenate((x, y), axis=0) #행
...
np.random.randn(5) #정규분포에서 랜덤하게

np.random.randn(5, 4)

m, sigma = 10, 2

m + sigma*np.random.randn(5) # seed를 다시 실행하고 이 라인을 실행하면 같은 수가 나온다.
...
unique_values = np.unique(a) #중복제거
